...
class DealershipAdmin(admin.ModelAdmin):
    ...
    def check_buttons(self, request, obj=None, **kwargs):
        ...
        dealership_opts, field_opts = None, None
        form = None
        selected_dealerships_list = []
        selected_fields_list = []
        fields_taken = False

        if get_field_button is None and update_button is None:
            dealership_opts, field_opts = self.get_select_lists_opts(request)
            fields_taken = False
        elif update_button is None:  # get form
            dealership_opts, field_opts = self.get_select_lists_opts(request)
            fields_taken = True
    ...
    def get_select_lists_opts(self, request):
